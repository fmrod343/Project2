...
class StudentGradeManager:
    ...
    def display_grades(self):
        ...
        try:
            ...
            return [["Error", "Unable to read CSV", str(e)]]
...
class GradeManagerApp:
    ...
    def add_student(self):
        if self.attempts >= self.max_attempts:
            messagebox.showerror("Error", "Maximum attempts reached. Closing application.")
            self.root.quit()
            return
        ...
        try:
            scores = list(map(float, filter(None, scores_input.split(","))))
    ...
    def on_close(self):
        ...
        if messagebox.askyesno("Exit", "Are you sure you want to exit? All unsaved data will be lost."):
            self.root.destroy()
